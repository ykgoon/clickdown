...
impl TuiLayout {
    ...
    pub fn render_too_small_warning(&self, frame: &mut Frame) {
        let warning = Paragraph::new(vec![
            ...
            Line::from(format!(
                "Current size: Please resize to at least {}x{}",
...
pub fn calculate_scroll_indicator(
    area: Rect,
    ...
    let thumb_size =
        ((visible_height * visible_height) as f64 / content_height as f64).ceil() as u16;
...
pub fn render_scroll_indicator(
    ...
    if let Some((thumb_start, thumb_size)) =
        calculate_scroll_indicator(area, content_height, scroll_offset)
    {
...
mod tests {
    ...
    fn test_split_task_detail_ratio() {
        ...
        assert!(
            desc_ratio >= 0.25 && desc_ratio <= 0.35,
            "Description ratio should be ~30%, got {}",
            desc_ratio
        );
    }
}
