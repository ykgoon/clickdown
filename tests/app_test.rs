...
async fn test_workspace_selection() {
    ...
    let _ = app.update(Message::Initialize);
    let _ = app.update(Message::WorkspacesLoaded(vec![test_workspace()]));

    // Select the workspace
    let workspace = test_workspace();
    let _ = app.update(Message::WorkspaceSelected(workspace.clone()));
...
async fn test_space_selection() {
    ...
    let _ = app.update(Message::WorkspaceSelected(test_workspace()));

    // Select space
    let space = test_space();
    let _ = app.update(Message::SpaceSelected(space.clone()));
...
async fn test_folder_selection() {
    ...
    let _ = app.update(Message::WorkspacesLoaded(vec![test_workspace()]));
    let _ = app.update(Message::WorkspaceSelected(test_workspace()));
    let _ = app.update(Message::SpacesLoaded(vec![test_space()]));
    let _ = app.update(Message::SpaceSelected(test_space()));
    let _ = app.update(Message::FoldersLoaded(vec![test_folder()]));

    // Select folder
    let folder = test_folder();
    let _ = app.update(Message::FolderSelected(folder.clone()));
...
async fn test_list_selection_and_task_loading() {
    ...
    let _ = app.update(Message::SpaceSelected(test_space()));
    let _ = app.update(Message::FoldersLoaded(vec![test_folder()]));
    let _ = app.update(Message::FolderSelected(test_folder()));
    let _ = app.update(Message::ListsLoaded(vec![test_list()]));

    // Select list
    let list = test_list();
    let _ = app.update(Message::ListSelected(list.clone()));
    
    // Simulate tasks loaded from async operation
    let _ = app.update(Message::TasksLoaded(test_tasks()));
...
async fn test_task_selection() {
    ...
    let _ = app.update(Message::FolderSelected(test_folder()));
    let _ = app.update(Message::ListsLoaded(vec![test_list()]));
    let _ = app.update(Message::ListSelected(test_list()));
    ...
    let _ = app.update(Message::TaskSelected(task.clone()));
...
async fn test_create_task_flow() {
    ...
    let _ = app.update(Message::CreateTaskRequested);
...
async fn test_delete_task() {
    ...
    // Simulate tasks loaded from async operation
    let _ = app.update(Message::TasksLoaded(tasks.clone()));

    // Verify tasks are loaded
    assert_eq!(app.tasks().len(), 3);

    // Delete task
    let _ = app.update(Message::DeleteTask(task_id.clone()));
    
    // Simulate task deleted confirmation from async operation
    let _ = app.update(Message::TaskDeleted(task_id.clone()));
...
async fn test_api_error_handling() {
    ...
    // Simulate error from async task
    app.update(Message::AuthError("API connection failed".to_string()));
